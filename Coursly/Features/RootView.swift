...
struct RootView: View {
    ...
    private func panelView(_ panel: FloatingPanel) -> some View {
        ...
        case .search:
            SearchPanel(
                onSelect: { event in store.goTo(event: event); self.panel = nil },
        ...
        case .more:
            MorePanel(
                ...
                onNewEvent: { self.panel = nil; showNewEvent = true },
                onSettings: { self.panel = nil; showSettings = true },
                onClose: { self.panel = nil }
            ).environment(store)
        case .changes:
            ChangeHistoryPanel(
...
struct CourslyBackdrop: View {
    var body: some View {
        ZStack {
            ...
            LinearGradient(
                colors: [Color.accentColor.opacity(0.12), Color.clear, Color.indigo.opacity(0.06)],
            ...
            Group {
                ...
                case .day:
                    DayTimelineView { event in
                        selectedEvent = event
                        HapticService.fire(.courseOpened, enabled: store.hapticsEnabled)
                    }
                case .week:
