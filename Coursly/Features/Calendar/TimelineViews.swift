...
struct DayTimelineView: View {
    ...
    private func handleScrollFeedback(offset: CGFloat) {
        ...
        let crossedCourse = crossedCourseBoundary(from: previous, to: offset, events: store.events(on: store.focusedDate), hourHeight: hourHeight)
        ...
        ScrollViewReader { proxy in
            ScrollView(.vertical, showsIndicators: false) {
                GeometryReader { geometry in
                    ...
                    let naturalWidth = max(geometry.size.width, CGFloat(days.count) * 104 + timelineTimeWidth + 8)
        ...
        ZStack(alignment: .topLeading) {
            ...
            ForEach(Array(days.enumerated()), id: \.offset) { dayIndex, day in
                ...
                ForEach(placements) { placement in
                    ...
                    let exactHeight = yPosition(for: placement.event.end, day: day) - yPosition(for: placement.event.start, day: day)
                    let height = max(1, exactHeight)
            ...
            HStack(spacing: 0) {
                ForEach(days, id: \.self) { day in
                    Button {
                        ...
                        .foregroundStyle(Calendar.current.isDate(day, inSameDayAs: now) ? Color.accentColor : Color.primary)
                    }
                    .buttonStyle(.plain)
...
struct CourseBlock: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            ...
            .font(.system(size: availableWidth < 115 ? 8 : 9, weight: .semibold, design: .rounded).monospacedDigit())
            .foregroundStyle(.secondary)
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .stroke(highlighted ? Color.accentColor : eventColor.opacity(0.38), lineWidth: highlighted ? 2.5 : 0.8)
        }
    ...
    private var accessibilityText: String {
        var parts = [event.type?.rawValue, event.title].compactMap { $0 }
