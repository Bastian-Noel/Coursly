...
struct EventLayoutEngine {
    func placements(for events: [CalendarEvent]) -> [EventPlacement] {
        ...
        func flushCluster() {
            ...
            output.append(contentsOf: assignments.map { EventPlacement(event: $0.0, column: $0.1, columnCount: columnCount) })
            cluster.removeAll(keepingCapacity: true)
            clusterEnd = nil
        }

        for event in ordered {
            if let end = clusterEnd, event.start >= end { flushCluster() }
...
struct DayTimelineView: View {
    ...
    var body: some View {
        GeometryReader { viewport in
            ScrollViewReader { proxy in
                ...
                .onChange(of: store.timelineRecenterToken) { _, _ in scrollToRelevantTime(proxy: proxy, animated: true, forceNow: true) }
    ...
    private func timelinePage(for date: Date, width: CGFloat) -> some View {
        DayTimelineCanvas(date: date, events: store.events(on: date), hourHeight: CGFloat(store.hourHeight), highlightedEventID: calendar.isDate(date, inSameDayAs: store.focusedDate) ? store.highlightedEventID : nil, onSelect: onSelect)
            .frame(width: width)
    }
        ...
        DragGesture(minimumDistance: 12)
            ...
            .onEnded { value in
                ...
                guard abs(effective) > 58, abs(horizontal) > abs(vertical) * 1.08 else {
                    withAnimation(.snappy(duration: 0.18)) { pageOffset = 0 }; return
                }
                let direction = effective < 0 ? 1 : -1
                isChangingDay = true
                var capture = Transaction(); capture.disablesAnimations = true
                withTransaction(capture) { pageOffset = max(-width, min(width, horizontal)) }
    ...
    private func handleScrollFeedback(from oldValue: CGFloat, to newValue: CGFloat) {
        ...
        let oldOffset = max(0, oldValue), newOffset = max(0, newValue)
        guard abs(newOffset - oldOffset) > 0.75 else { return }
        let hourHeight = CGFloat(store.hourHeight)
        let crossedCourse = crossedCourseBoundary(from: oldOffset, to: newOffset, events: store.events(on: store.focusedDate), hourHeight: hourHeight)
        if crossedCourse { HapticService.fire(.scrollCourse, enabled: true) }
        else if Int(oldOffset / hourHeight) != Int(newOffset / hourHeight) { HapticService.fire(.scrollHour, enabled: true) }
    }

    private func crossedCourseBoundary(from oldValue: CGFloat, to newValue: CGFloat, events: [CalendarEvent], hourHeight: CGFloat) -> Bool {
        let lower = min(oldValue, newValue), upper = max(oldValue, newValue)
        guard upper - lower < hourHeight * 2.5 else { return false }
        return courseBoundaryOffsets(events: events, hourHeight: hourHeight).contains { $0 > lower && $0 <= upper }
    }

    private func scrollToRelevantTime(proxy: ScrollViewProxy, animated: Bool, forceNow: Bool = false) {
        let event = forceNow ? nil : store.highlightedEventID.flatMap { id in store.events.first(where: { $0.id == id }) }
        ...
        let target = "day-quarter-\(max(0, min(95, Int(round(Double(minutes) / 15.0)))))"
        if animated { withAnimation(.snappy(duration: 0.38)) { proxy.scrollTo(target, anchor: .center) } }
...
struct DayTimelineCanvas: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            let placements = engine.placements(for: events), contentWidth = max(1, proxy.size.width - timelineTimeWidth)
            ZStack(alignment: .topLeading) {
                ...
                ForEach(placements) { placement in
                    ...
                    let cardHeight = max(1, max(1, exactBottom - exactTop) - eventVerticalInset * 2)
                    CourseBlock(event: placement.event, availableWidth: laneWidth, height: cardHeight, highlighted: highlightedEventID == placement.event.id)
                        .frame(width: laneWidth, height: cardHeight).offset(x: x, y: exactTop + eventVerticalInset).onTapGesture { onSelect(placement.event) }
                }
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ForEach(0...24, id: \.self) { hour in
                ...
                Rectangle().fill(Color.secondary.opacity(hour % 3 == 0 ? 0.24 : 0.15)).frame(height: hour % 3 == 0 ? 0.8 : 0.5).offset(x: timelineTimeWidth, y: y)
                Text(String(format: "%02d:00", hour % 24)).font(.caption2.monospacedDigit()).foregroundStyle(.secondary).frame(width: timelineTimeWidth - 8, alignment: .trailing).offset(x: 0, y: y - 7)
            }
            ForEach(0..<96, id: \.self) { quarter in Color.clear.frame(width: 1, height: 1).offset(y: CGFloat(quarter) * hourHeight / 4).id("day-quarter-\(quarter)") }
        }.frame(height: hourHeight * 24 + 2, alignment: .top)
    }
}
...
struct WeekTimelineView: View {
    ...
    var body: some View {
        GeometryReader { viewport in
            let dayWidth = max(1, (viewport.size.width - timelineTimeWidth) / 5), initialStart = initialWeekStart
            let days = extendedVisibleDays(centeredAround: initialStart), initialFive = fiveDays(startingAt: initialStart)
                            ...
                            ScrollView(.horizontal, showsIndicators: false) {
                                LazyHStack(alignment: .top, spacing: 0) {
                                    ForEach(days, id: \.self) { day in
                                        WeekDayColumn(day: day, events: store.events(on: day), now: store.now, hourHeight: CGFloat(store.hourHeight), width: dayWidth, highlightedEventID: store.highlightedEventID, onSelect: onSelect).frame(width: dayWidth).id(day)
                                    }
                                }.scrollTargetLayout()
                            }
                            .scrollTargetBehavior(.viewAligned(limitBehavior: .always)).defaultScrollAnchor(.leading)
                            .onAppear { horizontalProxy.scrollTo(initialStart, anchor: .leading) }
                            .onChange(of: store.focusedDate) { _, _ in withAnimation(.snappy(duration: 0.28)) { horizontalProxy.scrollTo(initialWeekStart, anchor: .leading) } }
                        }
                    }
                }
                .onScrollGeometryChange(for: CGFloat.self) { $0.contentOffset.y } action: { oldValue, newValue in handleWeekScrollFeedback(from: oldValue, to: newValue) }
                .onAppear {
                    guard !hasPositioned else { return }; hasPositioned = true
                    scrollToEarliestCourse(in: initialFive, proxy: verticalProxy, animated: false)
                    Task { if let first = days.first { await store.ensureLoaded(around: first) }; if let last = days.last { await store.ensureLoaded(around: last) } }
                }
                .onChange(of: store.timelineRecenterToken) { _, _ in
                    if calendar.isDate(store.focusedDate, inSameDayAs: store.now) { scrollToMinute(minutesSinceMidnight(store.now), proxy: verticalProxy, animated: true) }
                    else { scrollToEarliestCourse(in: fiveDays(startingAt: initialWeekStart), proxy: verticalProxy, animated: true) }
                }.refreshable { await store.refresh() }
            }
        }
    }
    private var initialWeekStart: Date { store.visibleWeekDays(containing: store.focusedDate).first ?? store.focusedDate }
    private func extendedVisibleDays(centeredAround center: Date) -> [Date] {
        var previous: [Date] = [], cursor = center
        for _ in 0..<15 { cursor = store.adjacentVisibleDate(from: cursor, direction: -1); previous.append(cursor) }
        var following: [Date] = []; cursor = center
        for _ in 0..<25 { cursor = store.adjacentVisibleDate(from: cursor, direction: 1); following.append(cursor) }
        return Array(previous.reversed()) + [center] + following
    }
    private func fiveDays(startingAt start: Date) -> [Date] { var output = [start], cursor = start; while output.count < 5 { cursor = store.adjacentVisibleDate(from: cursor, direction: 1); output.append(cursor) }; return output }
    private func scrollToEarliestCourse(in days: [Date], proxy: ScrollViewProxy, animated: Bool) {
        let keys = Set(days.map { calendar.startOfDay(for: $0) }); let earliest = store.events.filter { keys.contains(calendar.startOfDay(for: $0.start)) }.map(\.start).min()
        scrollToMinute(earliest.map(minutesSinceMidnight) ?? 8 * 60, proxy: proxy, animated: animated)
    }
    private func scrollToMinute(_ minutes: Int, proxy: ScrollViewProxy, animated: Bool) {
        let target = "week-quarter-\(max(0, min(95, Int(floor(Double(minutes) / 15.0)))))"
        if animated { withAnimation(.snappy(duration: 0.38)) { proxy.scrollTo(target, anchor: .top) } } else { proxy.scrollTo(target, anchor: .top) }
    }
    private func handleWeekScrollFeedback(from oldValue: CGFloat, to newValue: CGFloat) {
        guard store.hapticsEnabled else { return }; let h = CGFloat(store.hourHeight)
        if Int(max(0, oldValue - weekHeaderHeight) / h) != Int(max(0, newValue - weekHeaderHeight) / h) { HapticService.fire(.scrollHour, enabled: true) }
    }
}
...
private struct WeekTimeColumn: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ZStack(alignment: .topLeading) {
                ForEach(0...24, id: \.self) { hour in Text(String(format: "%02d:00", hour % 24)).font(.caption2.monospacedDigit()).foregroundStyle(.secondary).frame(width: timelineTimeWidth - 8, alignment: .trailing).offset(y: CGFloat(hour) * hourHeight - 7) }
                ForEach(0..<96, id: \.self) { quarter in Color.clear.frame(width: 1, height: 1).offset(y: CGFloat(quarter) * hourHeight / 4).id("week-quarter-\(quarter)") }
            }.frame(height: hourHeight * 24 + 2, alignment: .top)
        }
    }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            Button { store.focusedDate = calendar.startOfDay(for: day); store.setDisplayMode(.day) } label: {
                VStack(spacing: 1) { Text(capitalizedWeekday(day)).font(.caption2.weight(.semibold)).lineLimit(1); Text(day.formatted(.dateTime.day())).font(.subheadline.monospacedDigit().weight(.bold)) }
                    .frame(maxWidth: .infinity, maxHeight: .infinity).foregroundStyle(calendar.isDate(day, inSameDayAs: now) ? Color.accentColor : Color.primary).contentShape(Rectangle())
            }.buttonStyle(.plain).frame(height: weekHeaderHeight)
            ZStack(alignment: .topLeading) {
                WeekDayGrid(hourHeight: hourHeight)
                ForEach(engine.placements(for: events)) { placement in
                    let totalGaps = eventGap * CGFloat(max(0, placement.columnCount - 1)), laneWidth = max(1, (width - 4 - totalGaps) / CGFloat(placement.columnCount))
                    let x = 2 + CGFloat(placement.column) * (laneWidth + eventGap), top = yPosition(for: placement.event.start, hourHeight: hourHeight), bottom = yPosition(for: placement.event.end, hourHeight: hourHeight)
                    let cardHeight = max(1, max(1, bottom - top) - eventVerticalInset * 2)
                    CourseBlock(event: placement.event, availableWidth: laneWidth, height: cardHeight, highlighted: highlightedEventID == placement.event.id, forceCompact: true).frame(width: laneWidth, height: cardHeight).offset(x: x, y: top + eventVerticalInset).onTapGesture { onSelect(placement.event) }
                }
                if calendar.isDate(day, inSameDayAs: now) { WeekCurrentTimeIndicator(y: yPosition(for: now, hourHeight: hourHeight), width: width) }
            }.frame(height: hourHeight * 24 + 2, alignment: .top)
...
private struct WeekDayGrid: View {
    let hourHeight: CGFloat
    var body: some View { ZStack(alignment: .topLeading) { ForEach(0...24, id: \.self) { hour in Rectangle().fill(Color.secondary.opacity(hour % 3 == 0 ? 0.24 : 0.14)).frame(height: hour % 3 == 0 ? 0.8 : 0.5).offset(y: CGFloat(hour) * hourHeight) } }.frame(height: hourHeight * 24 + 2, alignment: .top) }
}

struct CurrentTimeIndicator: View {
    @Environment(CalendarStore.self) private var store
    let hourHeight: CGFloat; let x: CGFloat; let width: CGFloat
    var body: some View { TimelineView(.periodic(from: .now, by: 30)) { context in let y = yPosition(for: store.effectiveNow(from: context.date), hourHeight: hourHeight); HStack(spacing: 0) { Circle().fill(Color.red).frame(width: 8, height: 8); Rectangle().fill(Color.red).frame(height: 1.5) }.frame(width: width, alignment: .leading).offset(x: x - 4, y: y - 4).accessibilityHidden(true) } }
}
private struct WeekCurrentTimeIndicator: View { let y: CGFloat; let width: CGFloat; var body: some View { HStack(spacing: 0) { Circle().fill(Color.red).frame(width: 7, height: 7); Rectangle().fill(Color.red).frame(height: 1.5) }.frame(width: width, alignment: .leading).offset(x: -3, y: y - 3).accessibilityHidden(true) } }
...
struct CourseBlock: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: availableWidth > 190 ? 4 : 2) {
                HStack(alignment: .firstTextBaseline, spacing: 3) {
                    ...
                    else if event.source == .local { Image(systemName: "person.crop.circle").font(.caption2) }
        ...
        }.clipped().background(Rectangle().fill(eventColor.opacity(event.source == .local ? 0.09 : 0.12))).overlay { if highlighted { Rectangle().stroke(Color.accentColor, lineWidth: 2) } }.contentShape(Rectangle()).accessibilityElement(children: .ignore).accessibilityLabel(accessibilityText)
    }
